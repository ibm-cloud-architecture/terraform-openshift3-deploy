...
locals {
    ...
    openshift_master_configmap  = "${join(", ", formatlist("{'name': '%v', 'labels': ['node-role.kubernetes.io/master=true'],  'edits': [{ 'key': 'nodeIP','value': '%v'}]}", var.master_hostname, var.master_private_ip))}"
    openshift_infra_configmap  = "${join(", ", formatlist("{'name': '%v', 'labels': ['node-role.kubernetes.io/infra=true'],  'edits': [{ 'key': 'nodeIP','value': '%v'}]}", var.infra_hostname, var.infra_private_ip))}"
    openshift_worker_configmap  = "${join(", ", formatlist("{'name': '%v', 'labels': ['node-role.kubernetes.io/compute=true'],  'edits': [{ 'key': 'nodeIP','value': '%v'}]}", var.app_hostname, var.app_private_ip))}"
    openshift_storage_configmap  = "${join(", ", formatlist("{'name': '%v', 'labels': ['node-role.kubernetes.io/compute=true'],  'edits': [{ 'key': 'nodeIP','value': '%v'}]}", var.storage_hostname, var.storage_private_ip))}"
}
...
data "template_file" "ansible_inventory_ibmcloud" {
  template = <<EOF
[OSEv3:children]
masters
etcd
...
openshift_node_groups=[${local.openshift_master_configmap}, ${local.openshift_infra_configmap}, ${local.openshift_worker_configmap}, ${local.openshift_storage_configmap}]
...
${var.storage["nodes"] == "0" ? "" : "[glusterfs]"}
${var.storage["nodes"] == "0" ? "" : "${join("\n", formatlist("%v.%v glusterfs_devices='[ %v ]' openshift_node_group_name='%v'", var.storage_hostname, var.domain, local.gluster_storage_devices, var.storage_hostname))}"}

[nodes]
${join("\n", formatlist("%v.%v openshift_node_group_name=\"%v\"", var.master_hostname, var.domain, var.master_hostname))}
${join("\n", formatlist("%v.%v openshift_node_group_name=\"%v\"", var.infra_hostname, var.domain, var.infra_hostname))}
${join("\n", formatlist("%v.%v openshift_node_group_name=\"%v\"", var.app_hostname, var.domain, var.app_hostname))}
${var.storage["nodes"] == "0" ? "" : "${join("\n", formatlist("%v.%v openshift_schedulable=True openshift_node_group_name=\"%v\"", var.storage_hostname, var.domain, var.storage_hostname))}"}
...
resource "null_resource" "copy_ansible_inventory_ibmcloud" {
    count = "${var.storageprovider == "ibmcloud" ? 1 : 0}"
    triggers = {
        inventory = "${data.template_file.ansible_inventory_ibmcloud.rendered}"
    }
    ...
    provisioner "file" {
        when = "create"
        content     = "${data.template_file.ansible_inventory_ibmcloud.rendered}"
        destination = "~/inventory.cfg"
    }
}
