...
resource "null_resource" "pre_install_cluster" {
  ...
  provisioner "remote-exec" {
    inline = [
      ...
      "/tmp/scripts/pre_install_steps.sh ${join(",",concat(var.app_private_ip,var.infra_private_ip,var.master_private_ip,var.storage_private_ip))} ${join(",",concat(var.app_hostname,var.infra_hostname,var.master_hostname,var.storage_hostname))} ${var.domain}"
    ]
  }
}
